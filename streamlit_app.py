...
st.markdown(f"""
    ...
    .user-profile {{
        ...
        background: rgba(255, 255, 255, 0.15) !important; /* 增加背景亮度 */
        padding: 8px 20px 8px 8px; border-radius: 60px;
        border: 1px solid rgba(255, 255, 255, 0.3); /* 强化边线感 */
        backdrop-filter: blur(15px) saturate(160%); /* 强化毛玻璃效果 */
        transition: all 0.4s ease; cursor: pointer; opacity: 0.8;
    }}
    .user-profile:hover {{ 
    ...
    .user-name {{ color: #ffffff !important; font-weight: 700; }}
    ...
    .normal-sn:hover {{ transform: scale(1.4) !important; z-index: 10000; background: #38bdf8 !important; color: #000 !important; box-shadow: 0 0 30px #38bdf8; text-decoration: none !important; }}
    
    /* 异常拦截 SN：橙色回归 */
    .error-sn-pill {{ background: rgba(245, 158, 11, 0.15); color: #f59e0b !important; border-color: rgba(245, 158, 11, 0.3); }}
    .error-sn-pill:hover {{ transform: scale(1.4) !important; z-index: 10000; background: #f59e0b !important; color: #000 !important; box-shadow: 0 0 30px #f59e0b; text-decoration: none !important; }}
