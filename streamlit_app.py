...
st.markdown(f"""
    ...
    .error-card {{ border-left: 6px solid #f59e0b; background: rgba(245, 158, 11, 0.02); }}
...
def process_sku_logic(uploaded_file):
    ...
    for index, row in df.iterrows():
        ...
        if len(data_pairs) == i_qty and i_qty > 0:
            ...
            all_error_rows.append({'SN': sn, 'Line': index+2, 'Reason': f"数量不符({len(data_pairs)}/{i_qty})", 'Content': g_text})
            
    return pd.DataFrame(all_normal_data), pd.DataFrame(all_error_rows)

# --- 3. 页面渲染 ---
upload_placeholder = st.empty()
uploaded_file = upload_placeholder.file_uploader("Upload", type=["xlsx"], key="main_up")
