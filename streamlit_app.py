...
st.markdown(f"""
    ...
    /* 修复：浮动效果与霓虹光效 */
    .wide-card:hover {{
        transform: translateY(-10px) !important; /* 强制位移 */
        background: rgba(56, 189, 248, 0.05) !important;
        border-color: rgba(56, 189, 248, 0.6) !important;
        box-shadow: 0 20px 40px rgba(0,0,0,0.4), 0 0 25px rgba(56, 189, 248, 0.3) !important;
    }}
    ...
    .stTabs [data-baseweb="tab"] {{
        height: 40px !important; padding: 0 30px !important; 
        border-radius: 50px !important; /* 强制圆角胶囊 */
    ...
    /* 💊 修复：SN 码药丸 */
    .sn-pill {{ 
        padding: 6px 18px; border-radius: 50px !important; /* 强制胶囊 */
    ...
    .normal-sn:hover {{ background: #38bdf8 !important; color: #000 !important; transform: scale(1.05); }}
    ...
    div.stButton > button:hover {{ border-color: #38bdf8 !important; box-shadow: 0 0 15px rgba(56, 189, 248, 0.2); }}
    ...
    with t1:
        if not v_df.empty:
            ...
            for i, cat in enumerate(cats):
                ...
                st.markdown(f'''<div class="wide-card normal-card" style="animation-delay: {delay}s;"><div style="flex:1;"><div style="color:#38bdf8; font-weight:900; font-size:1.6rem; margin-bottom:12px; letter-spacing:1px;">{cat}</div>{attr_html}</div><div style="display:flex; flex-wrap:wrap; gap:8px; justify-content:flex-end; max-width:400px;">{sn_html}</div></div>''', unsafe_allow_html=True)
