...
st.markdown(f"""
    ...
    /* ✨ [极限 HDR] 上传框呼吸动画：模拟高压霓虹灯 */
    @keyframes uploader-glow {{
        0% {{ 
            border-color: rgba(56, 189, 248, 0.2); 
            box-shadow: 0 0 10px color(display-p3 0.22 0.74 0.97 / 0.1); 
        }}
        50% {{ 
            border-color: color(display-p3 0.4 0.85 1); 
            /* 白色核心 + P3 强光 + 环境溢出光 */
            box-shadow: 0 0 15px #fff, 0 0 30px color(display-p3 0.22 0.74 0.97), 0 0 70px color(display-p3 0.22 0.74 0.97 / 0.4); 
        }}
        100% {{ 
            border-color: rgba(56, 189, 248, 0.2); 
            box-shadow: 0 0 10px color(display-p3 0.22 0.74 0.97 / 0.1); 
    ...
    @keyframes avatar-breathing {{
        ...
        50% {{ box-shadow: 0 0 25px 5px color(display-p3 0.22 0.74 0.97 / 0.9); transform: scale(1.05); }}
    ...
    .normal-card:hover {{ 
        ...
        border-color: color(display-p3 0.5 0.9 1); 
        box-shadow: 0 20px 50px rgba(0,0,0,0.6), inset 0 0 40px color(display-p3 0.22 0.74 0.97 / 0.6), inset 0 0 100px color(display-p3 0.22 0.74 0.97 / 0.3); 
    }}
    .error-card {{ border-left: 5px solid rgba(245, 158, 11, 0.4); }}
    .error-card:hover {{ 
        transform: translateY(-8px); 
        border-color: color(display-p3 1 0.7 0.2); 
        box-shadow: 0 20px 50px rgba(0,0,0,0.6), inset 0 0 40px color(display-p3 0.96 0.62 0.04 / 0.6), inset 0 0 100px color(display-p3 0.96 0.62 0.04 / 0.3); 
    }}

    /* 🏷️ SN 标签与 Tab 切换：极限发光 */
    ...
    .stTabs [data-baseweb="tab"][aria-selected="true"]:nth-child(1) {{ 
        color: #fff !important; border-color: color(display-p3 0.5 0.9 1) !important; 
        background: rgba(56, 189, 248, 0.25) !important; 
        box-shadow: 0 0 15px #fff, 0 0 45px color(display-p3 0.22 0.74 0.97), 0 0 90px color(display-p3 0.22 0.74 0.97 / 0.4) !important; 
    }}
    .stTabs [data-baseweb="tab"][aria-selected="true"]:nth-child(2) {{ 
        ...
        box-shadow: 0 0 15px #fff, 0 0 45px color(display-p3 0.96 0.62 0.04), 0 0 90px color(display-p3 0.96 0.62 0.04 / 0.4) !important; 
    }}
    ...
    [data-testid="stFileUploader"] {{
        ...
        background: rgba(12, 30, 61, 0.65) !important; border-radius: 24px !important; 
        padding: 20px !important; backdrop-filter: blur(30px) !important;
